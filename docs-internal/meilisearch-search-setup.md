...
#    임시 서브도메인 (매번 URL 바뀜)
/snap/bin/loclx tunnel http -t 3000 -s localdocswhatever
#    → public URL: https://localdocswhatever.loclx.io

# 3. tunnel URL로 docs-staging 스크레이프
gh workflow run meilisearch-scrape-docs.yml \
  --repo kvidai/kvidai-documentation \
  -f index_uid=docs-staging \
  -f start_url=https://localdocswhatever.loclx.io/
